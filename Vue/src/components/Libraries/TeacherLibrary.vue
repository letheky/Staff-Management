          ...
          <div :style="{ overflow: 'scroll', height: '100%' }">
            ...
            >
              ...
              <template v-slot:addTreeNodeIcon="slotProps">
                <span v-if="permission.cFull && !slotProps.model.showAddFolder" class="icon"
...
export default {
  ...
  methods: {
    ...
    // async addPromiseKeyValue(node) {
    //   await referencies.getReferenceByFolderID(node.id).then(res => {
    //     if (res.length > 0) node.showAddFolder = true
    //     else node.showAddFolder = false
    //     if (node.children) {
    //       for (const child of node.children) {
    //         this.addPromiseKeyValue(child)
    //       }
    //     }
    //   })
    // },
    ...
    //add showAddFolder to all the node
    // async addProp(data) {
    //   data.forEach(node => {
    //     this.addPromiseKeyValue(node)
    //   })
    // },
    ...
    async showAddFolder(node) {
      await referencies.getReferenceByFolderID(node.id).then(res => {
        node.libraryType = this.libraryType
        node.showAddFolder = res.length > 0 ? true : false
        if (node.children) {
          for (const child of node.children) {
            this.showAddFolder(child)
          }
        }
      })
    },
    ...
    async initState(node) {
      await referencies.getReferenceByFolderID(node.id).then(res => {
        if (res.length > 0) this.hasReference = true
        else this.hasReference = false
        node.libraryType = this.libraryType
        node.showAddFolder = this.hasReference
        if (node.children) {
          for (const child of node.children) {
            this.showAddFolder(child)
          }
    ...
    async showAdd(id) {
      await referencies.getSubFolderByFolderID(id).then(res => {
        if (res) this.hasSubFolder = true
        else this.hasSubFolder = false
      })
    },
