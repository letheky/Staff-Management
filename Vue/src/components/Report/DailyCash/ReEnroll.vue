      ...
      <template v-slot:body="{ items, headers }">
        ...
        <tbody v-else v-for="(data, index) in items" :key="index">
          <tr class="date-filter">
            <td colspan="7" v-on:click="data.isOpen = !data.isOpen" style="white-space: nowrap">
        ...
        <tfoot>
          ...
          <tr>
            <td colspan="7" style="font-weight: bold">Target</td>
            <td style="text-align: right">{{ numberWithCommas(filtered.targetPrePaid) }}</td>
            <td style="text-align: right">{{ numberWithCommas(filtered.targetAmount) }}</td>
            <td style="text-align: right">{{ numberWithCommas(filtered.targetOther) }}</td>
            <td style="text-align: right">{{ numberWithCommas(filtered.targetPaidAmt) }}</td>
          </tr>
          <tr>
            <td colspan="10" style="font-weight: bold">Average</td>
            <td style="text-align: right">{{ filtered.average }}</td>
          </tr>
        </tfoot>
      </template>
    </v-data-table>
    ...
    async init() {
      if (this.centerId != '' && this.fromDate != '' && this.toDate != '') {
        ...
        await report_management
          ...
          .then(res => {
            if (res.length > 0) {
              this.cashList = res.filter(d => d.studentType.includes('enrol'))
              const set = new Set(this.cashList.map(obj => obj.counselorname))
              const res1 = []
              let tempDateList = ['All']
              for (const name of set.keys()) {
                const datas = this.cashList
                  .filter(obj => obj.counselorname === name)
                  .map(obj => obj)
                let a = 0,
                  b = 0,
                  c = 0,
                ...
                tempDateList.push(this._dayjs(name).format('DD/MM/YYYY'))
              }
