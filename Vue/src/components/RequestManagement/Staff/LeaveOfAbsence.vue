      ...
      <v-col md="12">
        <div class="pa-10 pt-5">
          <div class="d-flex justify-space-between">
            <h2>Staff leave of absence request list</h2>
          </div>
          <v-row class="pt-5">
            <v-col md="3">
              <!-- <date-picker
          ...
          <div class="pt-2 pb-5">
            <v-btn
              ...
              class="mr-5"
              :disabled="
                selected.filter((x) => x.status == 'PENDING').length > 0
                  ? false
                  : true
              "
              ...
              "
            >
              <v-icon color="success" small>mdi-check</v-icon>Approve all
              selected requests
            </v-btn>
            ...
            <v-btn
              ...
              "
            >
              <v-icon color="error" small>mdi-close</v-icon>Reject all selected
              requests
            </v-btn>
          </div>
          <v-progress-linear
      ...
      <v-row>
        ...
        <v-col md="4" class="d-flex align-center justify-end">
          <h3 v-if="teacherlistabsence.length > 1">
            Total: {{ teacherlistabsence.length }} staffs
          </h3>
          <h3 v-else>Total: {{ teacherlistabsence.length }} staff</h3>
        </v-col>
...
export default {
  ...
  data() {
    return {
      ...
      recordID: "",
      recordIDS: [],
      fromDateOfStaffAbsenceRequest: localStorage.getItem(
        "fromDateOfStaffAbsenceRequest"
      ),
      toDateOfStaffAbsenceRequest: localStorage.getItem(
        "toDateOfStaffAbsenceRequest"
      ),
      isBackFromDetailStaffAbsenceRequest:
        localStorage.getItem("isBackFromDetailStaffAbsenceRequest") || null,
      errorDialog: false,
      message: message,
    };
  },
  created() {
    if (!this.isBackFromDetailStaffAbsenceRequest) {
      this.fromDate = dayjs()
        .add(-parseInt(dayjs().add(-1, "day").format("DD")), "day")
        .format("YYYY-MM-DD");
      this.toDate = dayjs().format("YYYY-MM-DD");
    } else {
      this.fromDate = this.fromDateOfStaffAbsenceRequest;
      this.toDate = this.toDateOfStaffAbsenceRequest;
    }
    this.init();
  },
  watch: {
  ...
  methods: {
    async init() {
      ...
      await center_managerment
        .leaveAdminAbsence(this.fromDate, this.toDate, 5)
        .then((res) => {
    ...
    confirmRejectOrApproved() {
      center_managerment
        .adminApprovedAbsence(this.recordID, this.approvedOrReject)
        .then((res) => {
          if (res) {
            this.init();
          }
    ...
    confirmMultiApproveOrReject() {
      ...
      center_managerment
        .adminApprovedAbsence(this.recordIDS, this.approvedOrReject)
        .then((res) => {
          if (res) {
            this.init();
            this.selected = [];
    ...
    viewDetail(teacherID, fromTime, toTime, recordID) {
      localStorage.setItem("fromDateOfStaffAbsenceRequest", this.fromDate);
      localStorage.setItem("toDateOfStaffAbsenceRequest", this.toDate);
      localStorage.removeItem("isBackFromDetailStaffAbsenceRequest");
      this.$router.push({
