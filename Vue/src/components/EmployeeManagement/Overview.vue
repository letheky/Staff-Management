            ...
            <v-tooltip v-if="roleid === 1" top>
              <template v-slot:activator="{ on, attrs }">
                <img
                  v-bind="attrs"
                  v-on="on"
                  @click="$refs.confirmDeleteTopic.open(), (deleteResearchID = item.employeeID)"
...
export default {
  ...
  data() {
    return {
      ...
      headers: [
        ...
        {
          text: 'Số năm đóng bảo hiểm',
          value: 'insuranceContribution',
          sortable: false,
          width: '10%',
        },
        {
          text: 'Giáo viên cấp cao',
          value: 'isHighLevelTeacher',
          sortable: false,
          width: '10%',
        },
        {
  ...
  methods: {
    ...
    async getCircular() {
      this.circularList = await employee.getAllEmployee()
    },
    async getInstructor() {
      this.instructorList = await employee.getAllEmployee()
    },
    confirmDeleteTopic() {
      this.deleteTopic()
    },
    async deleteTopic() {
      await employee.deleteEmployee(this.deleteResearchID).then(() => {
    ...
    async callDataWithLoading() {
      await employee.getAllEmployee().then(res => {
        if (res)
